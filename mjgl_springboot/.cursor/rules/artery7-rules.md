...
- **Vue.js**: 2.6.11
- **Thymeleaf**: 2.7.18
- **Artery组件库**: 基于Vue的组件库artery-component
...
### 组件与示例获取规则（必须）
- 组件列表、属性、事件、示例，必须通过 MCP 服务 `artery7-mcp-server` 实时获取，不得手写臆造。
- 允许调用的 MCP 接口：
...
### 前端代码规范
- 尽可能地使用Artery组件，可使用artery7-mcp-server获取使用方式，不可随意编辑
...
### Artery组件白名单与使用约束（基于 MCP：artery7-mcp-server）
- 尽可能使用以下组件（MCP 官方清单）：
...
### Artery 前端 JS 方法白名单（基于 MCP：artery7-mcp-server）
...
### CSS 工具类白名单（基于 MCP：artery7-mcp-server）
