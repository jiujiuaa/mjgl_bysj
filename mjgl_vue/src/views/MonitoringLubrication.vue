  ...
  <div class="monitoring-container">
    ...
    <div class="layout-main" v-back-to-top>
      <header class="top-header">
        <div class="top-title">监测与异常 - 润滑巡检</div>
        <div class="top-subtitle">录入并查看模具润滑类人工巡检数据</div>
      </header>
          ...
          <section>
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">润滑巡检记录列表</h3>
              ...
              <div class="card-body">
                ...
                <div v-if="listLoading" class="table-loading">润滑巡检记录加载中...</div>
                <div v-else>
                    ...
                    <table class="mold-table">
                      ...
                      <tbody>
                        <tr v-if="!page.list || page.list.length === 0">
                          <td colspan="7" class="empty-cell">暂无润滑巡检记录</td>
                        </tr>
                        <tr v-for="item in page.list" :key="item.id">
                          <td>{{ item.moldCode || '-' }}</td>
                          <td>{{ item.moldName || '-' }}</td>
                          <td>{{ item.oilLevelPercent ?? '-' }}</td>
                          <td>{{ item.pressureKpa ?? '-' }}</td>
...
const loadList = async () => {
  ...
  try {
    ...
    const res = await queryLubricationLogs(param, pageNum.value, pageSize.value)
    const data = res.data || {}
    page.list = data.list ?? []
    page.total = data.total ?? 0
    page.pages = data.pages ?? 0
  } catch (e) {
    errorMessage.value = e.message || '加载润滑巡检记录失败'
  } finally {
...
const handleSubmit = async () => {
  ...
  if (form.oilLevelPercent == null) {
    errorMessage.value = '请填写油位'
    return
  }
  ...
  try {
    ...
    if (recordId && selectedFiles.value.length) {
      await uploadBizFiles(
        ...
        {
        bizType: 'lubrication_log',
        fileType: 'inspection_photo',
        description: '润滑巡检记录图片',
        },
      )
    }
    successMessage.value = '润滑巡检记录已保存'
    setTimeout(() => {
      successMessage.value = ''
    }, 3000)
    resetForm()
    ...
    errorMessage.value = e.message || '保存润滑巡检记录失败'
  }
}
